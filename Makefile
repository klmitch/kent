...
test: $(FORMAT_TARG) lint vet test-only cover-test

cover: $(FORMAT_TARG) lint vet test-only $(COVER_HTML) cover-test

cover-test: $(COVER_OUT)
	$(OVERCOVER) --config $(COV_CONF) $(COV_ARG) --coverprofile $(COVER_OUT) $(PACKAGES)
